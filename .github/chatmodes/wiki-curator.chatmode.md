...
- Write outside `docs/wiki/wiki/`. Specifically, never edit or create files in `docs/wiki/sources/`, `docs/wiki/outputs/`, or anywhere else in the repo.
- Modify production code, tests, infrastructure, agent files, instruction files, prompt files, or `CLAUDE.md`.
...
The harness does not enforce these — this prompt body is the enforcement layer. State and re-state in your output that you respect them.

1. **Read scope.** You may read anywhere in the repo via `#codebase`.
2. **Write scope.** `#editFiles` is restricted to paths matching `docs/wiki/wiki/**`, with one narrow carve-out: you may flip status markers on existing lines of `docs/wiki/sources/_pending.md` (`[PENDING] → [COMPILED — YYYY-MM-DD by mode-A|mode-B]`). You may not add, delete, or reorder lines in `_pending.md`, and you may not touch any other file under `sources/`. Refuse any other write outside `docs/wiki/wiki/**`.
3. **`docs/wiki/sources/` is append-only and not yours to write.** It is populated by the TDD orchestrator (cycle archiving) and owner-decision capture. The single carve-out for `_pending.md` status markers (rule 2) is the only mutation allowed.
4. **`docs/wiki/outputs/` is empty until a later phase.** You may not write into it.
5. **Terminal access is read-only.** Never run a command that mutates filesystem or git state.
...
Invoked by the project's TDD orchestrator (e.g., a `triad.md` Phase 8.5 step) with one argument: a path to a freshly archived cycle report under `docs/wiki/sources/cycles/<issue-id>-<slug>.md`.
...
5. Identify candidates:
   ...
   - **Lesson update?** An anti-pattern caught by code review or security audit that is not in `lessons.md`.
...
6. Produce a diff proposal as a single Markdown report (see §5). The proposal MUST also include:
   ...
   - Appending an entry to `docs/wiki/wiki/_log.md` per §9.2.
   **No file writes yet.**
...
Invoked by `/specflow-wiki`, typically end-of-sprint. This is the Karpathy "linting" pass — the LLM asks itself *"what is missing?"* rather than the human asking the LLM.

**Token budget: full wiki tree.** Acceptable cost since this runs at most weekly.

Procedure (run the 14 steps in order; do not skip):
...
11. **Flow / runbook / dependency / policy / risk scan.** Walk the codebase + cycle reports for content that has not yet been distilled into the corresponding category:
    ...
    - **Risks** — security-audit findings in cycle reports without a `risks/` page; high-severity TODOs or FIXMEs that flag accepted risk; threat-model gaps surfaced by an ADR or policy without a corresponding risk entry. Risks with `status: open` surface in `_hot.md` register.
...
- Sections: Decisions (ADRs), Domain primers, Patterns, Interfaces, Flows, Runbooks, Dependencies, Policies, Risks, Glossary & lessons, Control files.
